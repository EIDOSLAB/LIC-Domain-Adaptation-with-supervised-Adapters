...
class GaussianConditional(EntropyModel):
    ...
    *tensorflow/compression*. See the `tensorflow documentation
    ...
    def update(self):
        multiplier = -self._standardized_quantile(self.tail_mass / 2)  # quantile standardizzato
        pmf_center = torch.ceil(self.scale_table * multiplier).int() # quantile  rispetto alle varie deviazioni standard (media zero)
        pmf_length = 2 * pmf_center + 1  # lunghezza degli intervalli per contenere tail_mass, uno per ogni deviazione standard 
